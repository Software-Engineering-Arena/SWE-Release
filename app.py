...
def create_monthly_metrics_plot(top_n=5):
    ...
    if top_n is not None and top_n > 0 and metrics.get('assistants'):
        # Calculate total releases for each assistant
        agent_totals = []
    ...
    for idx, agent_name in enumerate(assistants):
        ...
        # Only show bars for months where assistant has releases
        x_bars = []
        y_bars = []
...
def get_leaderboard_dataframe():
    ...
    print(f"Filtered out {filtered_count} assistants with 0 total releases")
...
print(f"\nStarting SWE Assistant Release Leaderboard")
...
with gr.Blocks(title="SWE Assistant Release Leaderboard", theme=gr.themes.Soft()) as app:
    gr.Markdown("# SWE Assistant Release Leaderboard")
    gr.Markdown(f"Track and compare total releases by SWE assistants")

    with gr.Tabs():

        # Leaderboard Tab
        with gr.Tab("Leaderboard"):
            gr.Markdown("*Statistics are based on total releases by assistants*")
            ...
            with gr.Group():
                ...
                gr.Markdown("*Shows total releases for the most active assistants*")
